...
! rq0 : mode radius of initial distribution (input in um)
! xmw : total water content (input in g/m**3)
...
subroutine trkern(n,isw,g,r,e, ck, ec)
      ...
      double precision, intent(in),dimension(n)    :: g,r,e
      double precision, intent(inout),dimension(n,n) :: ck,ec
    
      double precision, dimension(n)   :: rr, winf
      double precision, dimension(n,n) :: cck
      integer              :: i,j,k, jm, im, jp, ip
      double precision                 :: x0, effi
...
! terminal velocity of falling drops
      integer, intent(in)              :: n
      double precision, intent(in),dimension(n)    :: g,r,e
      double precision, intent(out),dimension(n)   :: rr
      double precision, intent(inout),dimension(n) :: winf

      double precision, dimension(7)  :: b    
      double precision, dimension(6)  :: c    
      double precision, dimension(20) :: rat
      double precision, dimension(15) :: r0
      integer            :: i,j,k
      double precision, parameter    :: pi=3.141592654
      double precision   :: x0, eta, xlamb, rhow, rhoa, grav, cunh, &
...
subroutine effic(g,r,e, ec)
!collision efficiencies of hall kernel
      integer, parameter   :: n=400
      double precision, intent(in),    dimension(n)   :: g,r,e
      double precision, intent(inout), dimension(n,n) :: ec
      double precision, dimension(21)    :: rat
      double precision, dimension(15)    :: r0
