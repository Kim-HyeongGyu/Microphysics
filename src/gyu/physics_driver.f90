...
contains
    ...
    subroutine compute_conc( dmb_dt, Nr, mass, update_mass )   !{{{
        ...
        select case (phy_adv_scheme)
            case (1)    ! 1: reassign
                call redistribution( Nr, mass, update_mass )
    ...
    subroutine redistribution( mass, next_m, Nr ) !{{{
        implicit none
        real, dimension(:) ,intent(in)    :: mass
        real, dimension(:) ,intent(in)    :: next_m 
        real, dimension(:) ,intent(inout) :: Nr
        ...
        next_N = 0.
        nbin   = size(next_N)
        ! TODO: Porting from ncl code (+ doc/redistribution/reassign.ncl)
