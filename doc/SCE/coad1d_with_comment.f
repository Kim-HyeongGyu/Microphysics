...
c read input variables
      ...
      read *,dt,rq0,xmw,scal,isw                c      _
      rq0=rq0*1.e-04                            c rq0: r  [um] -> [cm]
      xmw=xmw*1.d-3                             c [g m-3] -> [mg cm-3]
      xn0=4./3.*pi*1000.*exp(log(rq0)*3.)       c [mg]
      xn1=xmw/xn0                               c [ cm-3 ]
      dlnr=dlog(2.d0)/(3.*scal)                 c dlnr: dy = ln(alpha/3)
      ax=2.d0**(1.0/scal)                       c ax:   alpha = 2^(1/2)
c mass and radius grid
      e(1)=emin*0.5*(ax+1.)                     c e: Minimum mass x1=x/2
      r(1)=1000.*dexp(dlog(3.*e(1)/(4.*pi))/3.) c r: Convert mass [mg] -> radius [um]
      do i=2,n
         e(i)=ax*e(i-1)                         c Equation (4)
         r(i)=1000.*dexp(dlog(3.*e(i)/(4.*pi))/3.)  
      enddo
c initial mass distribution                     c       _
      x0=xn1/xn0                                c x0: L/x
      do i=1,n
         x1=e(i)
         g(i)=3.*x1*x1*x0*dexp(-x1/xn0)         c Equation (2) (25)
      enddo
c courant numbers
      call courant
c kernel
...
c multiply kernel with constant timestep and logarithmic grid distance
      do i=1,n
      do j=1,n
         ck(i,j)=ck(i,j)*dt*dlnr                c ck: Kernel, dlnr: delta y
      enddo
      enddo
c time integration
      tlmin=1.d-6                               c tlmin: variable for caluclate minute
      t=1.d-6                                   c t: [s] integrated time
      lmin=0                                    c lmin: minute time units
      do ij=1,nt
         t=t+dt
         tlmin=tlmin+dt
c collision
...
c lower and upper integration limit i0,i1
 ...
 2010 continue
      ...
      do j=i,i1
         k=ima(i,j)
         kp=k+1
         x0=ck(i,j)*g(i)*g(j)   c Equation (5,6) RHS 2
         x0=min(x0,g(i)*e(j))   c 
         if (j.ne.k) x0=min(x0,g(j)*e(i))   c
         gsi=x0/e(j)            c Equation (5) RHS 2
         gsj=x0/e(i)            c Equation (6) RHS 2
         gsk=gsi+gsj            c Equation (8)
         g(i)=g(i)-gsi          c Equation (5)
         g(j)=g(j)-gsj          c Equation (6)
         gk=g(k)+gsk            c Equation (10)
         if (gk.gt.gmin) then
            x1=dlog(g(kp)/gk+1.d-60)    c x1: log(w(i,j)) Equation (14)
            flux=gsk/x1*(dexp(0.5*x1)-dexp(x1*(0.5-c(i,j))))    c Equation (18)
            flux=min(flux,gk)   c Equation (21)
            g(k)=gk-flux        c Equation (11)
            g(kp)=g(kp)+flux    c Equation (11)
         endif
      enddo
      enddo
      return
      ...
      do j=i,n
         x0=e(i)+e(j)
         do k=j,n
            if (e(k).ge.x0.and.e(k-1).lt.x0) then       c Equation (9)
               if (c(i,j).lt.1.-1.d-08) then
                  kk=k-1
                  c(i,j)=dlog(x0/e(k-1))/(3.d0*dlnr)    c Equation (13)
               else
                  c(i,j)=0.
                  kk=k
               endif
